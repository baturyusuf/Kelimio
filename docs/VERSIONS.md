...
| Apache HttpComponents Core 5 | 5.4.3 | Spring dependency-management property and Gradle lockfile | Security floor for CVE-2026-54399 and CVE-2026-54428 while Spring Boot 3.5.16 still manages 5.3.6. |
...
| iOS | deployment target 15.0; Swift Package Manager; AppAuth-iOS 2.0.0 at `145104f5` | Xcode project/shared scheme, both `Package.resolved` files, `pubspec.lock`, and macOS CI | ADR-019 selects Flutter 3.44's default SwiftPM integration. A platform or dependency-manager change requires plugin, device reach, and product analysis plus an ADR. |
...
The current enforcement artifacts are present for the backend, Android, iOS, Flutter/Dart, Node workspaces, generated clients, and Terraform: modern Gradle wrappers, dependency locks, exact constraints, and exact CI setup pins. ADR-019 makes the checked-in SwiftPM-integrated Xcode project/shared scheme, both Xcode-generated `Package.resolved` files, and `mobile/pubspec.lock` authoritative for iOS; a `Podfile.lock` is required only if a future accepted change reintroduces CocoaPods. The unsigned macOS CI build rejects resolution drift and is the native reproducibility gate. Any new workspace must adopt the same enforcement policy before it can be treated as reproducible.
