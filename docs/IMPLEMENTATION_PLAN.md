...
Status: in progress; local/test intake, approval, deterministic Type-D composition, stored client-capability enforcement, unpublished-draft commit, impact-bound release activation/rollback services, paged release-aware reprojection, the ADR-014/ADR-015 resumable Flutter intake-to-publication operator, and the ADR-016 subsequent-release proof boundary are implemented. Initial publication and owner-scoped state-loss recovery pass real-service E2E. A real-PostgreSQL later-release publication/rollback scenario proves non-empty active-score reprojection with unchanged lifetime score; production enablement, tree/editor conflict authoring, large-page retry/dead reconciliation, and staging operations remain open.
...
- extend the proven non-Excel local/test subsequent-release boundary into the production tree editor with ETag conflict/diff semantics; never use Excel to update an existing course;
