...
Status: in progress; local/test intake, approval, deterministic Type-D composition, stored client-capability enforcement, unpublished-draft commit, impact-bound release activation/rollback services, paged release-aware reprojection, and the ADR-014 Flutter intake-to-publication operator are implemented. Initial publication passes real-service E2E; production enablement, process-death import recovery, tree/editor conflict authoring, subsequent-release creation, and non-empty rollback/reprojection evidence remain open.
...
- extend the local mobile intake/preview/approval/draft/impact/publication operator with process-death discovery, teacher tree/editor, validation, ETag conflicts, diff/reapply, and unsaved-change protection;
