...
Status: in progress. ADR-021 implements the controlled production initial-course path: managed teacher eligibility, versioned terms acceptance, generated mobile contracts, API outbox dispatch, separate worker IAM/database identity, private versioned KMS storage, digest-pinned ClamAV, and SQS-driven zero-to-one ECS scaling. Deployment and the real production workbook/malware/scale-down canaries remain open. The production course-tree editor, public onboarding/moderation/legal policy, and large-page retry/dead reconciliation also remain open.
