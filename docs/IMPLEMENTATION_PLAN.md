...
Status: in progress; all four question types, the 150-test Flutter suite, the isolated combined learning/publication Android E2E, and the repository-managed API 24/30/36 emulator smoke matrix run locally, while supported physical-device/performance coverage and the remaining student product are open.

Implemented foundation: the Type-D domain/controller/UI path provides an accessible two-stage non-drag interaction with RTL, focus, screen-reader, narrow-layout, and text-scale coverage. It never grades locally, retains no submitted or correct mapping in durable recovery or diagnostics, and rebuilds an empty board after restart unless ownership-scoped no-store reconciliation returns committed feedback. Catalog search/access filtering, invitation redemption, course detail/offline download, scoreless offline practice, and account/profile/privacy/notification controls are localized in Turkish, English, and Arabic; offline practice has Arabic RTL and 200% text-scale widget coverage. The teacher can open a read-only learner preview of the active release without starting an attempt, awarding score/energy, or marking correct answers; its Arabic RTL/200% test proves typed answers remain hidden. Mutation and asynchronous error surfaces map typed failures to localized safe guidance instead of rendering provider detail, causes, or raw exceptions. ADR-023 adds owner-only, active-release teacher analytics that hides partial projections and suppresses performance below a three-learner cohort. The mobile transport centrally advertises `question.matching.v1`; Flutter analysis and all 150 tests pass, including Type-D, resumable teacher, analytics privacy/RTL, secure editor recovery/conflict, and capability-header coverage. The combined learning, reviewed-workbook publication, and conflict-safe revision-3 authoring Android E2E passes against a fresh Flyway V13 stack using a per-run random replay key; and API 24/Nexus 5, API 30/Pixel 3a, and API 36/Pixel 7 smoke checks pass 15/15. Emulator evidence does not replace supported physical-device, native OIDC, update/migration, accessibility-audit, or performance gates.
...
Status: in progress. ADR-021's controlled production initial-course path is deployed: managed teacher eligibility, versioned terms acceptance, generated mobile contracts, API outbox dispatch, separate worker IAM/database identity, private versioned KMS storage, digest-pinned ClamAV, and SQS-driven zero-to-one ECS scaling are applied. The protected deployment, empty-queue worker/scanner Fargate health proof, scale-to-zero state, readiness, and no-drift plan pass. ADR-022's production course-tree API is deployed, and the mobile editor now retains the full local tree after a stale ETag, displays base/mine/latest multi-entity paths, and requires an explicit latest-or-reapply choice before saving. ADR-023's owner-scoped, active-release aggregate analytics is deployed through the progress-module application boundary with unresolved-projection suppression and a three-learner performance privacy floor. The 2026-08-29 guarded deployment built and scanned all three immutable images, completed the one-shot migration, promoted revision `429fb86e4d46173fb1e80084c9ce220fa373f610`, and passed workflow plus independent HTTPS readiness checks. The teacher course list, full editor, learner preview, and analytics are localized in Turkish, English, and Arabic, with Arabic RTL and 200% text-scale widget coverage; the broader product-wide localization/accessibility release gate remains open. The real production workbook/malware/archive/DLQ/natural scale-down and physical-device analytics/editor-conflict canaries remain open. Public onboarding/moderation/legal policy and large-page retry/dead reconciliation also remain open.
...
- retain production and physical-device evidence for owner-scoped teacher analytics, unresolved-projection failure behavior, and small-cohort suppression.
