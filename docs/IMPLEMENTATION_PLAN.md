...
Status: in progress; local/test approval-only intake is implemented, while production enablement, course commit, mobile authoring, and release remain open.

Foundation evidence: under ADR-010, the API exposes local/test-only create, status, upload-completion, preview, issues, and approval operations but no commit operation. It binds a canonical multipart plan and checksums to one owner, reconciles the exact versioned S3 object, appends dispatch through the transactional outbox/SQS, and uses bounded worker leases/retries plus a private ClamAV network. The worker retains immutable quarantine/archive/report and scanner/parser provenance, runs the bounded streaming parser/planner, exposes HMAC-bound paged results, and accepts approval only for one exact digest/provenance tuple. Approval creates no course, revision, release, entitlement, or publication, and import enablement fails outside `local`/`test`. The local starter's bounded `EV` matching fixture is not production import conversion: production Type-D group allocation remains fail-closed, and no stored minimum-client/capability publication gate exists yet. The Phase 3 deliverables below remain required before an import can create or change a course.

The isolated import acceptance runner passes the reviewed valid workbook through approval, a clean-invalid workbook through deterministic validation failure, and EICAR through malware rejection, while proving ownership isolation, immutable facts, queue drain, and zero course mutation. The retained normal stack migrated from V8 to V9 without fact-count drift and remains healthy.
