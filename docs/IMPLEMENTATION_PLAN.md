...
- extend the now-resumable local mobile intake/preview/approval/draft/impact/publication operator with teacher tree/editor validation, ETag conflicts, diff/reapply, and unsaved-change protection;
- add a non-Excel subsequent-release producer and exercise rollback across multiple immutable releases;
- retain staging evidence for paged idempotent reprojection with non-empty enrollment snapshots, retry, dead-job reconciliation, and cache invalidation;
