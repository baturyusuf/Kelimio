...
| LB-005 | The secure Excel preview core exists, but production import and mobile authoring are absent. | Retain the passing malicious-fixture and deterministic planner/digest evidence; add isolated upload/scanner/worker execution, immutable archive/provenance and approval binding, idempotent commit, conflict UI, release publication, rollback, and reprojection evidence. | Open |
